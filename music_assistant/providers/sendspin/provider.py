...
class SendspinProvider(PlayerProvider):
    ...
    def _pick_pairing_code_method(
        ...
        if not static:
            if dynamic is not None and "digits" in (dynamic.formats or ()):
                return PairMethod.DYNAMIC_PAIRING_CODE, "digits"
            if verify:
        ...
        if dynamic is not None and "digits" not in (dynamic.formats or ()):
            raise SecurityActionError("pairing_error_format_unsupported")
        raise SecurityActionError("pairing_error_no_pairing_code_method")
